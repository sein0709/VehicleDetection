"""Application settings loaded from environment variables."""

from __future__ import annotations

from pydantic_settings import BaseSettings, SettingsConfigDict
from shared_contracts.env import repo_env_file


class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        env_prefix="GREYEYE_",
        env_file=repo_env_file(),
        env_file_encoding="utf-8",
        case_sensitive=False,
        extra="ignore",
    )

    # Postgres (asyncpg)
    database_url: str = "postgresql://greyeye:greyeye_dev@localhost:5432/greyeye"

    # Redis (live KPI cache)
    redis_url: str = "redis://localhost:6379/0"

    # JWT (tokens issued by auth-service)
    jwt_secret: str = "dev-secret-change-in-production"
    jwt_algorithm: str = "HS256"

    # S3-compatible object storage for report exports
    s3_endpoint: str = "http://localhost:9000"
    s3_bucket: str = "greyeye-exports"
    s3_access_key: str = ""
    s3_secret_key: str = ""

    # Export / share settings
    export_ttl_seconds: int = 3600
    share_link_ttl_days: int = 7

    # Service
    service_name: str = "reporting-api"
    debug: bool = False

    # Observability
    log_level: str = "INFO"
    json_logs: bool = True
    tracing_enabled: bool = True
    otlp_endpoint: str = "http://localhost:4317"


_settings: Settings | None = None


def get_settings() -> Settings:
    global _settings
    if _settings is None:
        _settings = Settings()
    return _settings
